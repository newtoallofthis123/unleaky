...
while True:
	if master_password_verify(2, given_password) == "verified":
		...
		if action == "clear":
			...
			print("UNLEAKY\nBy NoobScience")
		if action == "version":
			print("You are currently using Unleaky version v.1.1\n Unleaky is a Cli Password Manager registered under the MIT License. By NoobScience")
		if action == "quit":
			...
			exit()
		if action == "destroy":
			choice_to_destroy = str.lower(input("Are you sure you want to destroy the password database and reset?(yes/no): "))
			if choice_to_destroy == "yes":
				choice_to_quit = str.lower(input("This action cannot be undone. You will lose all your saved passwords. Are you sure?(yes/no): "))
				if choice_to_quit == "yes":
					try:
						os.remove("unleaky.db")
					except:
						print("This cannot be done from Unleaky, quit unleaky and run destroy.py/destroy.exe")
				elif choice_to_quit == "no":
					print("Okay, quitting...")
					exit()
				else:
					print(f'Invalid command, {choice_to_quit}, action failed; (yes/no)')
			elif choice_to_destroy == "no":
				print("Okay, quitting...")
				exit()
			else:
				print(f'Invalid command, {choice_to_destroy}, action failed; (yes/no)')	
	else:
		print("Wrong Master Password, quiting application")
		break
# except:
